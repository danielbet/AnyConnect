...
checkGccVersion()
{
    gcc_in_proc_version=0
    version_string=`cat /proc/version`
    if [[ $version_string == *"gcc version"* ]]; then
        gcc_in_proc_version=`echo ${version_string} | awk -F'gcc version ' '{ print $2 }' | awk -F'.' '{ print $1"."$2 }'`
    elif [[ $version_string == *"gcc"* ]]; then
        gcc_in_proc_version=`echo ${version_string} | awk -F' ' '{ print $8 }' | awk -F'.' '{ print $1"."$2 }'`
    fi
...
rebuildAcKdfKo()
{
    ...
    ret=$?
    if [ ${ret} == 0 ]; then
        # AnyConnect Kernel module is not present
        return 3
    fi
    if [ ${ret} == 2 ]; then
        # AnyConnect Kernel module is compatible with current kernel release.
