...
LOGFNAME=`/bin/date "+posture-4.10.00093-install-%H%M%S%d%m%Y.log"` ||
  LOGFNAME=`/usr/bin/date "+posture-4.10.00093-install-%H%M%S%d%m%Y.log"` ||
  LOGFNAME=`date "+posture-4.10.00093-install-%H%M%S%d%m%Y.log"` ||
  LOGFNAME=posture-4.10.00093-install.log
...
echo "Installing Cisco AnyConnect Posture Module v4.10.00093..." > /tmp/${LOGFNAME}
